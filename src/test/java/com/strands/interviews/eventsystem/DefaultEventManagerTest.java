...
public class DefaultEventManagerTest {
	...
	public void testRegisterListenerAndPublishEvent() {
		EventListenerMock eventListenerMock = new EventListenerMock(
				new Class[] { SimpleEvent.class });
	...
	@Test
	public void testRegisterListenerAndPublishSubEvent() {
		EventListenerMock subeventListenerMock = new EventListenerMock(
				new Class[] { SubEvent.class });
	...
	public void testUnregisterListener() {
		EventListenerMock eventListenerMock = new EventListenerMock(
				new Class[] { SimpleEvent.class });
		EventListenerMock eventListenerMock2 = new EventListenerMock(
				new Class[] { SimpleEvent.class });
	...
	public void testRemoveNonexistentListener() {
		...
		eventManager.registerListener("some.key", new EventListenerMock(
				new Class[] { SimpleEvent.class }));
	...
	/**
	 * Registering duplicate keys on different listeners should only fire the
	 * most recently added.
	 */
	@Test
	...
	public void testAllEventsListener() {

		EventListenerMock eventListenerMock = new EventListenerMock(new Class[] { SubEvent.class });

		eventManager.registerListener("all.events",eventListenerMock);
		eventManager.publishEvent(new SimpleEvent(this));

		DefaultEventManager evManager = (DefaultEventManager) eventManager;
		assertTrue(evManager.isAllEventsListenerInstalled());
		AllEventsListener allEventsListener = (AllEventsListener) evManager.getAllEventsListener();
		assertEquals(1L, allEventsListener.getCount());
	}

	/**
	 * Attempting to register a null with a valid key should result in an
